...
void DRW_Entity::extrudePoint(DRW_Coord extPoint, DRW_Coord *point) {
  double px, py, pz;
  px = (extAxisX.x * point->x) + (extAxisY.x * point->y)
       + (extPoint.x * point->z);
  py = (extAxisX.y * point->x) + (extAxisY.y * point->y)
       + (extPoint.y * point->z);
  pz = (extAxisX.z * point->x) + (extAxisY.z * point->y)
       + (extPoint.z * point->z);
...
//            curr.code = code;
      ...
      if (code == 330 || code == 360) curr.addInt(code,
                                                  reader->getHandleString()); //RLZ code or nc
      else {
        switch (reader->type) {
...
bool DRW_Entity::parseDwg(DRW::Version version,
                          dwgBuffer *buf,
                          dwgBuffer *strBuf,
                          duint32 bs) {
  objSize = 0;
...
// RLZ: TODO
  ...
  if (entmode == 0) ownerHandle = true;
//        entmode = 2;
  else if (entmode == 2) entmode = 0;
...
//ENC color
  ...
  if (version > DRW::AC1014) { //2000+
    ...
    for (duint8 i = 0; i < 2; ++i) { //two flags in one
      ...
      if (plotFlags == 1) plotStyleName = "byblock";
      else if (plotFlags == 2) plotStyleName = "continuous";
      else if (plotFlags == 0) plotStyleName = "bylayer";
      else //handle at end
      plotStyleName = "";
      if (i == 0) {
        ltFlags = plotFlags;
...
bool DRW_Entity::parseDwgEntHandle(DRW::Version version, dwgBuffer *buf) {
  ...
  if (ownerHandle) { //entity are in block or in a polyline
    ...
    DRW_DBGHL(ownerH.code, ownerH.size, parentHandle);DRW_DBG("\n");
  }
  else
  ...
  for (int i = 0; i < numReactors; ++i) {
    ...
    DRW_DBGHL(reactorsH.code, reactorsH.size, reactorsH.ref);DRW_DBG("\n");
  }
  if (xDictFlag != 1) { //linetype in 2004 seems not have XDicObjH or NULL handle
    dwgHandle XDicObjH = buf->getHandle();
    DRW_DBG(" XDicObj control Handle: ");
    DRW_DBGHL(XDicObjH.code, XDicObjH.size, XDicObjH.ref);DRW_DBG("\n");
  }
  DRW_DBG("Remaining bytes: ");
  DRW_DBG(buf->numRemainingBytes());
  DRW_DBG("\n");
  ...
  if (version < DRW::AC1015) { //R14-
    ...
    if (ltFlags == 3) {
      ...
      DRW_DBG("\n");
      DRW_DBG("   Remaining bytes: ");
      DRW_DBG(buf->numRemainingBytes());DRW_DBG("\n");
    }
  }
...
bool DRW_Point::operator ==(const DRW_Entity &o) const {
  if (&o == this) return true;
  if (o.type() != type()) return false;
  DRW_Point &other = (DRW_Point&) o;

  if (basePoint == other.basePoint) return true;
  return false;
}
...
bool DRW_Line::operator ==(const DRW_Entity &o) const {
  if (&o == this) return true;
  if (o.type() != type()) return false;
  DRW_Line &other = (DRW_Line&) o;

  if (secPoint == other.secPoint) return DRW_Point::operator ==(other);
  return false;
}
...
bool DRW_Circle::operator ==(const DRW_Entity &o) const {
  if (&o == this) return true;
  if (o.type() != type()) return false;
  DRW_Circle &other = (DRW_Circle&) o;

  if (radious == other.radious) return DRW_Point::operator ==(other);
  return false;
}
...
bool DRW_Arc::operator ==(const DRW_Entity &o) const {
  if (&o == this) return true;
  if (o.type() != type()) return false;
  DRW_Arc &other = (DRW_Arc&) o;

  if (staangle == other.staangle) {
    if (endangle == other.endangle) {
      if (isccw == other.isccw) return DRW_Circle::operator ==(other);
    }
  }
  return false;
}
...
void DRW_Ellipse::toPolyline(DRW_Polyline *pol, int parts) {
  ...
  do {
    ...
    double x = basePoint.x + (cosCurr * cosRot * radMajor)
        - (sinCurr * sinRot * radMinor);
    double y = basePoint.y + (cosCurr * sinRot * radMajor)
               + (sinCurr * cosRot * radMinor);
...
bool DRW_3Dface::parseDwg(DRW::Version v, dwgBuffer *buf, duint32 bs) {
  ...
  }drw_assert(invisibleflag>=NoEdge);drw_assert(invisibleflag<=AllEdges);
...
bool DRW_Insert::parseDwg(DRW::Version version, dwgBuffer *buf, duint32 bs) {
  ...
  if (hasAttrib) {
    if (version < DRW::AC1018) { //2000-
      ...
      DRW_DBGHL(attH.code, attH.size, attH.ref);DRW_DBG("\n");
    }
    else {
    ...
    DRW_DBGHL(seqendH.code, seqendH.size, seqendH.ref);DRW_DBG("\n");
  }
  DRW_DBG("   Remaining bytes: ");
  DRW_DBG(buf->numRemainingBytes());
  DRW_DBG("\n");
...
void DRW_LWPolyline::parseCode(int code, dxfReader *reader) {
  switch (code) {
    ...
    case 20:
      if (vertex != NULL) vertex->y = reader->getDouble();
      break;
    case 40:
      if (vertex != NULL) vertex->stawidth = reader->getDouble();
      break;
    case 41:
      if (vertex != NULL) vertex->endwidth = reader->getDouble();
      break;
    case 42:
      if (vertex != NULL) vertex->bulge = reader->getDouble();
      break;
    case 38:
...
bool DRW_LWPolyline::parseDwg(DRW::Version version,
                              dwgBuffer *buf,
                              duint32 bs) {
  bool ret = DRW_Entity::parseDwg(version, buf, NULL, bs);
  if (!ret) return ret;
  ...
  DRW_DBG("flags value: ");
  DRW_DBG(flags);
  if (flags & 4) width = buf->getBitDouble();
  if (flags & 8) elevation = buf->getBitDouble();
  if (flags & 2) thickness = buf->getBitDouble();
  if (flags & 1) extPoint = buf->getExtrusion(false);
  vertexnum = buf->getBitLong();
  vertlist.reserve(vertexnum);
  unsigned int bulgesnum = 0;
  if (flags & 16) bulgesnum = buf->getBitLong();
  int vertexIdCount = 0;
  if (version > DRW::AC1021) { //2010+
    if (flags & 1024) vertexIdCount = buf->getBitLong();
  }

  unsigned int widthsnum = 0;
  if (flags & 32) widthsnum = buf->getBitLong();
  DRW_DBG("\nvertex num: ");
  DRW_DBG(vertexnum);
  DRW_DBG(" bulges num: ");
  DRW_DBG(bulgesnum);
...
//                DRW_Vertex2D *pv = vertlist.back();
    ...
    for (unsigned int i = 0; i < bulgesnum; i++) {
      ...
      if (vertlist.size() > i) vertlist.at(i)->bulge = bulge;
    }
    //add vertexId
    if (version > DRW::AC1021) {        //2010+
...
//                    vertlist.at(i)->vertexId = vertexId;
  ...
  if (DRW_DBGGL == DRW_dbg::DEBUG) {
    DRW_DBG("\nVertex list: ");
    for (std::vector<DRW_Vertex2D*>::iterator it = vertlist.begin();
        it != vertlist.end(); ++it) {
      DRW_Vertex2D *pv = *it;
      DRW_DBG("\n   x: ");
      DRW_DBG(pv->x);
      ...
      DRW_DBG(" stawidth: ");
      DRW_DBG(pv->stawidth);
      DRW_DBG(" endwidth: ");DRW_DBG(pv->endwidth);
    }
  }
...
bool DRW_Polyline::parseDwg(DRW::Version version, dwgBuffer *buf, duint32 bs) {
  ...
  if (oType == 0x0F) { //pline 2D
    ...
    DRW_DBG(tmpFlag);
    if (tmpFlag & 1) curvetype = 5;
    else if (tmpFlag & 2) curvetype = 6;
    if (tmpFlag & 3) {
      curvetype = 8;
      flags |= 4;
    }
    tmpFlag = buf->getRawChar8();
    if (tmpFlag & 1) flags |= 1;
...
//0x0D PFACE
//0x0E PFACE FACE
bool DRW_Vertex::parseDwg(DRW::Version version,
                          dwgBuffer *buf,
                          duint32 bs,
                          double el) {
  bool ret = DRW_Entity::parseDwg(version, buf, NULL, bs);
  ...
  if (oType == 0x0A) { //pline 2D, needed example
    ...
    else endwidth = buf->getBitDouble();
...
bool DRW_Hatch::parseDwg(DRW::Version version, dwgBuffer *buf, duint32 bs) {
  ...
  if (version > DRW::AC1015) { //2004+
    ...
    DRW_DBG("\ngradient name: ");
    DRW_DBG(gradName.c_str());DRW_DBG("\n");
  }
  basePoint.z = buf->getBitDouble();
  extPoint = buf->get3BitDouble();
  DRW_DBG("base point: ");
  ...
  for (dint32 i = 0; i < loopsnum; ++i) {
    ...
    if (!(loop->type & 2)) { //Not polyline
      ...
      for (dint32 j = 0; j < numPathSeg; ++j) {
        ...
        if (typePath == 1) { //line
          ...
          for (dint32 j = 0; j < spline->ncontrol; ++j) {
            ...
            if (isRational) crd->z = buf->getBitDouble(); //RLZ: investigate how store weight
            spline->controllist.push_back(crd);
          }
          if (version > DRW::AC1021) { //2010+
      ...
      for (dint32 j = 0; j < numVert; ++j) {
        ...
        if (asBulge) v.bulge = buf->getBitDouble();
        pline->addVertex(v);
      }
      loop->objlist.push_back(pline);
    } //end polyline
...
void DRW_Spline::parseCode(int code, dxfReader *reader) {
  switch (code) {
    ...
    case 20:
      if (controlpoint != NULL) controlpoint->y = reader->getDouble();
      break;
    case 30:
      if (controlpoint != NULL) controlpoint->z = reader->getDouble();
      break;
    case 11: {
    ...
    case 21:
      if (fitpoint != NULL) fitpoint->y = reader->getDouble();
      break;
    case 31:
      if (fitpoint != NULL) fitpoint->z = reader->getDouble();
      break;
    case 40:
...
//        DRW_DBG("unk bit: "); DRW_DBG(buf->getBit());
  ...
  if (scenario == 2) {
    ...
    DRW_DBG(ncontrol);
    DRW_DBG(" weight bit: ");DRW_DBG(weight);
  }
  else {
  ...
  if (DRW_DBGGL == DRW_dbg::DEBUG) {
    DRW_DBG("\nknots list: ");
    for (std::vector<double>::iterator it = knotslist.begin();
        it != knotslist.end(); ++it) {
      DRW_DBG("\n");DRW_DBG(*it);
    }
    DRW_DBG("\ncontrol point list: ");
    for (std::vector<DRW_Coord*>::iterator it = controllist.begin();
        it != controllist.end(); ++it) {
      DRW_DBG("\n");DRW_DBGPT((*it)->x, (*it)->y, (*it)->z);
    }
    DRW_DBG("\nfit point list: ");
    for (std::vector<DRW_Coord*>::iterator it = fitlist.begin();
        it != fitlist.end(); ++it) {
      DRW_DBG("\n");DRW_DBGPT((*it)->x, (*it)->y, (*it)->z);
    }
  }
...
bool DRW_Dimension::parseDwg(DRW::Version version,
                             dwgBuffer *buf,
                             dwgBuffer *sBuf) {
  DRW_DBG(
      "\n***************************** parsing dimension *********************************************");
  ...
  if (version > DRW::AC1014) { //2000+
    ...
    DRW_DBG(buf->getBit());
    DRW_DBG(buf->getBit());
    DRW_DBG(buf->getBit());DRW_DBG(buf->getBit());
  }
...
bool DRW_DimAligned::parseDwg(DRW::Version version,
                              dwgBuffer *buf,
                              duint32 bs) {
  dwgBuffer sBuff = *buf;
  dwgBuffer *sBuf = buf;
  ...
  if (oType == 0x15) setAn50(buf->getBitDouble() * ARAD);
  else type |= 1;
  DRW_DBG("\n  type (70) final: ");
  DRW_DBG(type);
  DRW_DBG("\n");
...
bool DRW_DimDiametric::parseDwg(DRW::Version version,
                                dwgBuffer *buf,
                                duint32 bs) {
  dwgBuffer sBuff = *buf;
  dwgBuffer *sBuf = buf;
...
bool DRW_DimAngular::parseDwg(DRW::Version version,
                              dwgBuffer *buf,
                              duint32 bs) {
  dwgBuffer sBuff = *buf;
  dwgBuffer *sBuf = buf;
...
bool DRW_DimAngular3p::parseDwg(DRW::Version version,
                                dwgBuffer *buf,
                                duint32 bs) {
  dwgBuffer sBuff = *buf;
  dwgBuffer *sBuf = buf;
...
bool DRW_DimOrdinate::parseDwg(DRW::Version version,
                               dwgBuffer *buf,
                               duint32 bs) {
  dwgBuffer sBuff = *buf;
  dwgBuffer *sBuf = buf;
...
void DRW_Leader::parseCode(int code, dxfReader *reader) {
  switch (code) {
    ...
    case 20:
      if (vertexpoint != NULL) vertexpoint->y = reader->getDouble();
      break;
    case 30:
      if (vertexpoint != NULL) vertexpoint->z = reader->getDouble();
      break;
    case 340:
...
bool DRW_Leader::parseDwg(DRW::Version version, dwgBuffer *buf, duint32 bs) {
  ...
  if (version < DRW::AC1024) { //2010-
    ...
    DRW_DBG("\ntextheight ");
    DRW_DBG(textheight);
    DRW_DBG(" textwidth ");DRW_DBG(textwidth);
  }
  hookline = buf->getBit();
  arrow = buf->getBit();
  DRW_DBG(" hookline ");
...
bool DRW_Viewport::parseDwg(DRW::Version version, dwgBuffer *buf, duint32 bs) {
  ...
  if (version > DRW::AC1014) { //2000+
    ...
    DRW_DBG("\n");
    DRW_DBG("Circle zoom?: ");
    DRW_DBG(buf->getBitShort());DRW_DBG("\n");
  }
  if (version > DRW::AC1018) { //2007+
    DRW_DBG("Grid major?: ");
    DRW_DBG(buf->getBitShort());DRW_DBG("\n");
...
//        DRW_DBG("Ambient Cmc or Enc: "); DRW_DBG(buf->getCmColor(version)); DRW_DBG("\n");
    ...
    DRW_DBG(buf->getEnColor(version));DRW_DBG("\n");
  }
  ret = DRW_Entity::parseDwgEntHandle(version, buf);
  ...
  if (version < DRW::AC1015) { //R13 & R14 only
    ...
    DRW_DBGHL(someHdl.code, someHdl.size, someHdl.ref);DRW_DBG("\n");
  }
  if (version > DRW::AC1014) { //2000+
