...
public:
  DRW_Entity(const DRW_Entity &e)
      : eType(e.eType),
        ...
        haveNextLinks(e.haveNextLinks),
        plotFlags(e.plotFlags),
        ltFlags(e.ltFlags),
  ...
  virtual bool operator ==(const DRW_Entity&) const {
    return false;
  }
  void reset() {
    for (std::vector<DRW_Variant*>::iterator it = extData.begin();
...
protected:
  ...
  bool parseDwg(DRW::Version version,
                dwgBuffer *buf,
                dwgBuffer *strBuf,
                duint32 bs = 0);
...
class DRW_Point: public DRW_Entity
{
  SETENTFRIENDS
protected:
  DRW_Point(DRW::ETYPE t)
      : DRW_Entity(t), basePoint(0, 0, 0), thickness(0), extPoint(0, 0, 1) {
  }
public:
  DRW_Point()
      : DRW_Entity(DRW::POINT),
        basePoint(0, 0, 0),
        thickness(0),
        extPoint(0, 0, 1) {
  }
  virtual bool operator ==(const DRW_Entity &o) const;
...
class DRW_Line: public DRW_Point
{
  SETENTFRIENDS
protected:
  DRW_Line(DRW::ETYPE t)
      : DRW_Point(t), secPoint(0, 0, 0) {
  }

public:
  DRW_Line()
      : DRW_Point(DRW::LINE), secPoint(0, 0, 0) {
  }
  virtual void applyExtrusion() {
  }
  virtual bool operator ==(const DRW_Entity &o) const;
...
class DRW_Circle: public DRW_Point
{
  SETENTFRIENDS
protected:
  DRW_Circle(DRW::ETYPE t)
      : DRW_Point(t), radious(0) {
  }

public:
  DRW_Circle()
      : DRW_Point(DRW::CIRCLE), radious(0) {
  }
  virtual void applyExtrusion();
  virtual bool operator ==(const DRW_Entity &o) const;
...
public:
  DRW_Arc()
      : DRW_Circle(DRW::ARC), staangle(0), endangle(0), isccw(1) {
  }
  virtual void applyExtrusion();
  virtual bool operator ==(const DRW_Entity &o) const;
  //! center point in OCS
...
public:
  DRW_Ellipse()
      : DRW_Line(DRW::ELLIPSE), ratio(0), staparam(0), endparam(0), isccw(1) {
    isccw = 1;
  }
  void toPolyline(DRW_Polyline *pol, int parts = 128);
  virtual void applyExtrusion();
  virtual bool operator ==(const DRW_Ellipse &o) const {
    if (ratio == o.ratio) {
      if (staparam == o.staparam) {
        if (endparam == o.endparam) {
          if (isccw == o.isccw) return DRW_Line::operator ==(o);
        }
      }
    }
    return false;
...
class DRW_Trace: public DRW_Line
{
  SETENTFRIENDS
protected:
  DRW_Trace(DRW::ETYPE t)
      : DRW_Line(t), thirdPoint(0, 0, 0), fourPoint(0, 0, 0) {
  }

public:
  DRW_Trace()
      : DRW_Line(DRW::TRACE), thirdPoint(0, 0, 0), fourPoint(0, 0, 0) {
  }
  virtual void applyExtrusion();
...
public:
  ...
  DRW_3Dface()
      : DRW_Trace(DRW::E3DFACE), invisibleflag(0) {
  }
  virtual void applyExtrusion() {
  }
  //! first corner in WCS
...
public:
  DRW_Block()
      : DRW_Point(DRW::BLOCK), name("*U0"), flags(0), isEnd(false) {
    layer = "0";
  }
...
public:
  ...
  DRW_MText()
      : DRW_Text(DRW::MTEXT), interlin(1), haveXAxis(false) {
...
private:
  bool haveXAxis;   //if true needed to recalculate angle
};
...
public:
  ...
  ~DRW_Spline() {
#ifdef TRY2FIX
    for (std::vector<DRW_Coord*>::const_iterator it = controllist.begin();
        it != controllist.end(); ++it)
      if (*it) delete *it;
    for (std::vector<DRW_Coord*>::const_iterator it = fitlist.begin();
        it != fitlist.end(); ++it)
      if (*it) delete *it;
#endif
  }
...
public:
  ...
  DRW_DimAligned(const DRW_Dimension &d)
      : DRW_Dimension(d) {
  }
...
public:
  ...
  DRW_DimLinear(const DRW_Dimension &d)
      : DRW_DimAligned(d) {
  }
...
public:
  ...
  DRW_DimRadial(const DRW_Dimension &d)
      : DRW_Dimension(d) {
  }
...
public:
  ...
  DRW_DimDiametric(const DRW_Dimension &d)
      : DRW_Dimension(d) {
  }
...
public:
  ...
  DRW_DimAngular(const DRW_Dimension &d)
      : DRW_Dimension(d) {
  }
...
public:
  ...
  DRW_DimAngular3p(const DRW_Dimension &d)
      : DRW_Dimension(d) {
  }
...
public:
  ...
  DRW_DimOrdinate(const DRW_Dimension &d)
      : DRW_Dimension(d) {
  }
