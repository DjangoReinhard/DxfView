...
public:
  ...
  bool operator ==(const DRW_Coord &o) const {
...
public:
  DRW_Vertex2D()
      : x(0), y(0), stawidth(0), endwidth(0), bulge(0) {
  }

  DRW_Vertex2D(double sx, double sy, double b)
      : x(sx), y(sy), stawidth(0), endwidth(0), bulge(b) {
  }
...
//TODO: add INT64 support
  ...
  DRW_Variant(int c, UTF8STRING s)
      : sdata(s), vdata(0, 0, 0), content(&sdata), vType(STRING), vCode(c) {
  }
...
private:
  union DRW_VarContent
  {
    ...
    DRW_VarContent(UTF8STRING *sd)
        : s(sd) {
    }
    DRW_VarContent(dint32 id)
        : i(id) {
    }
    DRW_VarContent(double dd)
        : d(dd) {
    }
    DRW_VarContent(DRW_Coord *vd)
        : v(vd) {
    }
  };
...
public:
  ...
  static enum lineWidth dxfInt2lineWidth(int i) {
    if (i < 0) {
      if (i == -1) return widthByLayer;
      else if (i == -2) return widthByBlock;
      else if (i == -3) return widthDefault;
    }
    else if (i < 3) {
      return width00;
    }
