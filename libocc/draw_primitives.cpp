...
// Draw 3d solids
Handle(AIS_Shape) OCCT::draw_3d_cone(gp_Pnt centerpoint, double bottomdiameter, double topdiameter, double height) {
  gp_Dir axis=gp::DX();
...
Handle(AIS_Shape) OCCT::draw_3d_tcp_cone(gp_Pnt centerpoint, double bottomdiameter, double topdiameter, double height) {
  gp_Dir axis=-gp::DX();
  gp_Ax2 aplace(centerpoint,axis);
  TopoDS_Shape t_topo_cone = BRepPrimAPI_MakeCone(aplace,bottomdiameter,topdiameter,height).Shape();
  return new AIS_Shape(t_topo_cone);
...
// Draw 2d primitives:
Handle(AIS_Shape) OCCT::draw_2d_circle(gp_Pnt center,double radius) {
  gp_Dir dir(0,0,1);
...
Handle(AIS_Shape) OCCT::draw_cp_2d_arc(gp_Pnt center, gp_Pnt point1, gp_Pnt point2) {
...
Handle(AIS_Shape) OCCT::draw_2d_acad_arc(gp_Pnt center, double radius, const Standard_Real alpha1, const Standard_Real alpha2) {
  gp_Dir dir(0,0,1); // you can change this
  gp_Circ circle(gp_Ax2( center, dir),radius);
...
Handle(AIS_Shape) OCCT::draw_2d_ellipse(gp_Pnt center, gp_Pnt secpoint, double alpha_start, double alpha_end, double ratio) {
  ...
  // Convert offset values to absolute coordinates.
  secpoint.SetX(secpoint.X()+center.X());
  secpoint.SetY(secpoint.Y()+center.Y());
  secpoint.SetZ(secpoint.Z()+center.Z());
...
// Draw 3d tools:
Handle(AIS_Shape) OCCT::draw_3d_origin(gp_Pnt origin, double linelenght) {
...
Handle(AIS_Shape) OCCT::rotate_3d(Handle(AIS_Shape) shape, gp_Pnt center, double euler_z, double euler_y, double euler_x) {
...
Handle(AIS_Shape) OCCT::rotate_translate_3d_quaternion(Handle(AIS_Shape) shape, gp_Pnt translation, double euler_z, double euler_y, double euler_x) {
...
Handle(AIS_Shape) OCCT::translate_3d(Handle(AIS_Shape) shape, gp_Pnt current, gp_Pnt target) {
  gp_Trsf trsf;
...
//! Transparancy 0.0 is not visible
Handle(AIS_Shape) OCCT::colorize(Handle(AIS_Shape) shape, Quantity_Color color, double transparancy) {
...
// Draw 3d primitives:
Handle(AIS_Shape) OCCT::draw_3d_point(gp_Pnt point) {
  TopoDS_Vertex vertex = BRepBuilderAPI_MakeVertex(point);
  return new AIS_Shape(vertex);
}

Handle(AIS_Shape) OCCT::draw_3d_line(gp_Pnt point1, gp_Pnt point2) {
  // Check if linelenght>0
  double l=OCCT::get_3d_line_lenght(point1,point2);
  TopoDS_Edge edge;
...
Handle(AIS_Shape) OCCT::draw_3d_line_wire(std::vector<gp_Pnt> points) {
...
Handle(AIS_Shape) OCCT::draw_3p_3d_arc(gp_Pnt point1,gp_Pnt point2,gp_Pnt point3) {
  Handle(Geom_TrimmedCurve) aArcOfCircle = GC_MakeArcOfCircle(point1,point2,point3);
  TopoDS_Edge aEdge = BRepBuilderAPI_MakeEdge(aArcOfCircle);
  return new AIS_Shape(aEdge);
}

Handle(AIS_Shape) OCCT::draw_3p_3d_circle(gp_Pnt point1,gp_Pnt point2,gp_Pnt point3) {
  Handle(Geom_Circle) aCircle = GC_MakeCircle(point1,point2,point3);
  TopoDS_Edge aEdge2 = BRepBuilderAPI_MakeEdge(aCircle);
  return new AIS_Shape(aEdge2);
}

Handle(AIS_Shape) OCCT::draw_3d_spline(std::vector<gp_Pnt> pointvec, int divisions) {
...
Handle(AIS_Shape) OCCT::draw_2d_text(std::string str, int textheight, gp_Pnt point, double rot_deg) { //https://www.youtube.com/watch?v=31SXQLpdNyE
...
Handle(AIS_Shape) OCCT::draw_3d_point_origin_cone(gp_Pnt point, gp_Pnt euler) {
...
Handle(AIS_Shape) OCCT::draw_3d_point_origin_cone_text(gp_Pnt point, gp_Pnt euler, std::string text, int textheight, int textrotation) {
...
Handle(AIS_Shape) OCCT::draw_3d_line_origin_cone_text(gp_Pnt point1, gp_Pnt point2, gp_Pnt euler1, gp_Pnt euler2, std::string text, int textheight) {

  // Draw the line
  Handle(AIS_Shape) Ais_shape=draw_3d_line(point1,point2);
  Ais_shape=OCCT().colorize(Ais_shape,Quantity_NOC_BLUE,0);

  // Draw the first origin
  Handle(AIS_Shape) Ais_child=OCCT().draw_3d_origin( {0,0,0},25);
  ...
  Ais_child=OCCT().draw_3d_origin( {0,0,0},50);
  ...
  // Draw the first cone
  Ais_child=OCCT().draw_3d_tcp_cone( {0,0,0},0,5,25);
  Ais_child=OCCT().colorize(Ais_child,Quantity_NOC_GREEN,0.5);
  Ais_child=OCCT().rotate_translate_3d_quaternion(Ais_child,point1,
      euler1.Z(),
      euler1.Y(),
      euler1.X());
  Ais_shape->AddChild(Ais_child);
  ...
  Ais_child=OCCT().draw_3d_tcp_cone( {0,0,0},0,5,25);
  Ais_child=OCCT().colorize(Ais_child,Quantity_NOC_BLACK,0.5);
  Ais_child=OCCT().rotate_translate_3d_quaternion(Ais_child,point2,
      euler2.Z(),
      euler2.Y(),
      euler2.X());
  Ais_shape->AddChild(Ais_child);
  ...
  // Draw the text id.
  Handle(AIS_Shape) Ais_text=OCCT::draw_2d_text(text, textheight, midpoint(point1,point2), 0);
  Ais_text=colorize(Ais_text,Quantity_NOC_BLACK,0.5);
  Ais_shape->AddChild(Ais_text);
...
Handle(AIS_Shape) OCCT::draw_3d_wire_origin_cone_text(std::vector<gp_Pnt> points, std::vector<gp_Pnt> euler, std::string text, int textheight) {
  BRepBuilderAPI_MakeWire wire;
  for(unsigned int i=0; i<points.size()-1; i++) {
    TopoDS_Edge edge = BRepBuilderAPI_MakeEdge(points.at(i),points.at(i+1));
    wire.Add(edge);
  ...
  for(unsigned int i=0; i<points.size()-1; i++) {
    ...
    Handle(AIS_Shape) Ais_text=OCCT::draw_2d_text(subidtext, textheight, OCCT::midpoint(points.at(i),points.at(i+1)), 0);
    Ais_text=colorize(Ais_text,Quantity_NOC_BLACK,0.5);
    Ais_shape->AddChild(Ais_text);
  }
...
Handle(AIS_Shape) OCCT::draw_3d_arc_origin_cone_text(std::vector<gp_Pnt> points, std::vector<gp_Pnt> euler, std::string text, int textheight) {
...
Handle(AIS_Shape) OCCT::draw_3d_circle_origin_cone_text(std::vector<gp_Pnt> points, std::vector<gp_Pnt> euler, std::string text, int textheight) {
...
Handle(AIS_Shape) OCCT::draw_3d_spline_origin_cone_text(std::vector<gp_Pnt> points, std::vector<gp_Pnt> euler, int divisions, std::string text, int textheight) {
...
gp_Pnt OCCT::midpoint(gp_Pnt point1, gp_Pnt point2) {
  gp_Pnt midpoint;
...
std::vector<Handle(AIS_Shape)> OCCT::draw_3d_arc_lenght(gp_Pnt point1,
                                                        gp_Pnt point2,
                                                        gp_Pnt point3) {
  ...
  aArcOfCircle = GC_MakeArcOfCircle(point1, point2, point3);
...
double OCCT::get_3d_line_lenght(gp_Pnt point1, gp_Pnt point2) {
  return sqrt(
      pow(point2.X() - point1.X(), 2) + pow(point2.Y() - point1.Y(), 2)
      + pow(point2.Z() - point1.Z(), 2));
}

double OCCT::get_3d_arc_lenght(gp_Pnt point1,
                               gp_Pnt point2,
                               gp_Pnt point3,
                               int divisions) {
...
gp_Pnt OCCT::get_3d_arc_point_given_arclenght_fromstartpoint(gp_Pnt point1,
                                                             gp_Pnt point2,
                                                             gp_Pnt point3,
...
Quantity_Color OCCT::autocad_color(int colornr) {
  ...
  switch (colornr) {
    ...
    case 4: // Cyan
      return Quantity_NOC_CYAN;
    case 5: // Blue
      return Quantity_NOC_BLUE;
    case 6: // Magenta
      return Quantity_NOC_MAGENTA;
    case 7: // White
...
std::vector<gp_Pnt> OCCT::get_cp_2d_acad_arc_points(gp_Pnt center,
                                                    double radius,
                                                    double alpha1,
                                                    double alpha2) {

  double pi_angle_half = (alpha2 + alpha1) / 2;

  gp_Pnt start, control, end;
  std::vector<gp_Pnt> pointvec;

  start.SetX((radius * cosf(alpha1)) + center.X());
  start.SetY((radius * sinf(alpha1)) + center.Y());
  start.SetZ(center.Z());

  control.SetX((radius * cosf(pi_angle_half)) + center.X());
  control.SetY((radius * sinf(pi_angle_half)) + center.Y());
  control.SetZ(center.Z());

  end.SetX((radius * cosf(alpha2)) + center.X());
  end.SetY((radius * sinf(alpha2)) + center.Y());
  end.SetZ(center.Z());

  pointvec.push_back(start);
  pointvec.push_back(control);
  pointvec.push_back(end);
  return pointvec;
}

std::vector<gp_Pnt> OCCT::get_cirlce_circumfence_points(gp_Pnt center,
                                                        double radius,
                                                        int division) {
  ...
  GeomAdaptor_Curve acrv(aArcOfCircle);
  TColgp_Array1OfPnt pkte(0, division);
  GCPnts_UniformAbscissa algo(acrv, division);
  if (algo.IsDone() && algo.NbPoints() > 0) {
    // int n = algo.NbPoints();
    // std::cout<<"total points:"<<n<<std::endl;
    // std::cout<<"abscissa dist:"<<algo.Abscissa()<<std::endl; // Distance between the points. Totalcurvelenght = (points-1)*abscissa dist.

    for (int i = 1; i < algo.NbPoints() + 1; i++) { // Clockwise output of points. Startpoint + Endpoint is included.
      double param = algo.Parameter(i);
      gp_Pnt p = aArcOfCircle->Value(param);
      // std::cout<<"Px:"<<p.X()<< " Py:" << p.Y() << " Pz:" << p.Z() <<std::endl;
...
std::vector<gp_Pnt> OCCT::get_arc_circumfence_points(gp_Pnt center,
                                                     double radius,
                                                     double alpha1,
                                                     double alpha2,
                                                     int division) {
  ...
  gp_Circ circle(gp_Ax2(center, dir), radius);
  Handle (Geom_TrimmedCurve)
  aArcOfCircle = GC_MakeArcOfCircle(circle, alpha1, alpha2, 0);
...
std::vector<gp_Pnt> OCCT::get_ellipse_circumfence_points(gp_Pnt center,
                                                         gp_Pnt secpoint,
                                                         double alpha_start,
                                                         double alpha_end,
                                                         double ratio,
  ...
  //center point to endpoint mayor axis..
  double MayorRadius = sqrt(
      pow(secpoint.X() - center.X(), 2) + pow(secpoint.Y() - center.Y(), 2)
      + pow(secpoint.Z() - center.Z(), 2));
  //ratio minor axis to mayor axis..
  double MinorRadius = ratio * MayorRadius;

  gp_Dir xDirection(secpoint.X() - center.X(), secpoint.Y() - center.Y(),
                    secpoint.Z() - center.Z()); // Direction is auto normalized by occ.
  gp_Dir normalDirection(0, 0, 1);
  gp_Ax2 axis(center, normalDirection, xDirection);

  gp_Elips ellipse(axis, MayorRadius, MinorRadius);
  Handle (Geom_TrimmedCurve)
  aArcOfEllipse = GC_MakeArcOfEllipse(ellipse, alpha_start, alpha_end, 0);

  GeomAdaptor_Curve acrv(aArcOfEllipse);
  TColgp_Array1OfPnt pkte(0, division);
  GCPnts_UniformAbscissa algo(acrv, division);
  if (algo.IsDone() && algo.NbPoints() > 0) {
    // int n = algo.NbPoints();
    ...
    for (int i = 1; i < algo.NbPoints() + 1; i++) { // Clockwise output of points. Startpoint + Endpoint is included.
      double param = algo.Parameter(i);
      gp_Pnt p = aArcOfEllipse->Value(param);
      std::cout << "Px:" << p.X() << " Py:" << p.Y() << " Pz:" << p.Z()
      << std::endl;
      pointvec.push_back(p);
    }
