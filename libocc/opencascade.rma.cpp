...
OpenCascade::OpenCascade(QWidget *parent)
 : QGLWidget(parent) {
...
void OpenCascade::get_selections() { // Updated by jointpos function from mainwindow.
  ...
  for (m_context->InitSelected()
     ; m_context->MoreSelected()
     ; m_context->NextSelected()) {
      std::cout << "iterate selection?!?" << std::endl;
      for (unsigned int i = 0; i < contourvec.size(); i++) {
          for (unsigned int j = 0; j < contourvec.at(i).primitive_sequence.size(); j++) {
              if (m_context->SelectedShape()
                  == contourvec.at(i).primitive_sequence.at(j).ashape->Shape()) {
  ...
  if (select) {
     m_context->AddOrRemoveSelected(contourvec.at(selected_contour)
                                              .primitive_sequence
                                              .at(temp).ashape
                                  , 1); // De-select the clicked item, un-highlight.
     for (unsigned int j = 0
        ; j < contourvec.at(selected_contour).primitive_sequence.size()
        ; j++) {
         m_context->AddOrRemoveSelected(contourvec.at(selected_contour)
                                                  .primitive_sequence
                                                  .at(j)
                                                  .ashape
                                      , 1); // Select the contour, hightlight it.
         }
     }
...
void OpenCascade::delete_selections() {
  ...
  for (unsigned int i = 0; i < datavec.size(); i++) {
      ...
      for (explorer.Init(datavec.at(i).ashape->Shape(), TopAbs_EDGE)
         ; explorer.More()
         ; explorer.Next()) {
          const TopoDS_Edge &edge = TopoDS::Edge(explorer.Current());
          ...
          std::cout << "content left edge p1 x: "
                    << p1.X() << " y:" << p1.Y() << " z:" << p1.Z() << std::endl;
          std::cout << "content left edge p2 x: "
                    << p2.X() << " y:" << p2.Y() << " z:" << p2.Z() << std::endl;
          }
      }
...
void OpenCascade::m_initialize_context() {
  if (m_context.IsNull()) {
     ...
     m_display_donnection = new Aspect_DisplayConnection();
     ...
     t_hilight_style->SetColor(Quantity_NOC_YELLOW);
...
//  aisViewCube->SetFixedAnimationLoop(false);
  ...
  aisViewCube->SetTransformPersistence(new Graphic3d_TransformPers(Graphic3d_TMF_TriedronPers
...
void OpenCascade::paintEvent(QPaintEvent*) {
  if (m_context.IsNull()) m_initialize_context();
  m_view->Redraw();
  }
...
void OpenCascade::wheelEvent(QWheelEvent *event) {
  ...
  m_view->ZoomAtPoint(0, 0, event->angleDelta().y() / 4, 0);
  }


void OpenCascade::switchOrthographic() {
...
void OpenCascade::setOrthographic() {
  m_view->Camera()->SetProjectionType(Graphic3d_Camera::Projection_Orthographic);
  m_view->Update();
  }


void OpenCascade::setPerspective() {
  m_view->Camera()->SetProjectionType(Graphic3d_Camera::Projection_Perspective);
  m_view->Redraw();
  }


void OpenCascade::setFrontView() {
  m_view->SetProj(V3d_Yneg);
  zoom2Fit();
  }


void OpenCascade::setBackView() {
  m_view->SetProj(V3d_Ypos);
  zoom2Fit();
  }


void OpenCascade::setTopView() {
  m_view->SetProj(V3d_Zpos);
  zoom2Fit();
  }


void OpenCascade::setBottomView() {
  m_view->SetProj(V3d_Zneg);
  zoom2Fit();
  }

void OpenCascade::setLeftView() {
  m_view->SetProj(V3d_Xneg);
  zoom2Fit();
  }


void OpenCascade::setRightView() {
